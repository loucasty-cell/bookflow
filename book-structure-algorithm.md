...
- Use a bounded concurrency worker pool during initial document OCR and extraction to prevent memory spikes on large documents, bounded to a maximum of 4 parallel workers based on `navigator.hardwareConcurrency`.
