...
def _render_pdf_batch_sync(pdf_bytes: bytes, batch_start: int, batch_size: int, force_ocr: bool = False) -> List[Dict[str, Any]]:
    """
    Synchronous high-speed CPU worker to extract a single batch of pages.
    Native text pages are processed in microseconds; scanned image pages are rendered at 96 DPI.
    Eliminates disk I/O bottlenecks and bounds memory.
    """
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    rendered_pages = []
    matrix = fitz.Matrix(RENDER_SCALE, RENDER_SCALE)

    end_idx = min(len(doc), batch_start + batch_size)
    for idx in range(batch_start, end_idx):
        ...
        # we can skip costly pixmap rasterization.
...
async def render_pdf_batch_async(pdf_bytes: bytes, batch_start: int, batch_size: int, force_ocr: bool = False) -> List[Dict[str, Any]]:
    """Asynchronously offload PDF batch rendering to thread pool executor."""
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(executor, _render_pdf_batch_sync, pdf_bytes, batch_start, batch_size, force_ocr)


def _check_pdf_requires_ocr_sync(pdf_bytes: bytes) -> bool:
    """Checks if any page in the first few pages needs OCR"""
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    requires_ocr = False
...
async def check_pdf_requires_ocr_async(pdf_bytes: bytes) -> bool:
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(executor, _check_pdf_requires_ocr_sync, pdf_bytes)
...
async def process_ocr_pipeline(
    ...
    1. Extracts pages in bounded batches to limit memory usage.
    2. Runs concurrent async calls to Hugging Face Inference API.
    3. Streams real-time page updates via SSE.
    """
    async with jobs_lock:
        job = jobs.get(job_id)
    if not job:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0, limits=limits) as http_client:
            requires_ocr = await check_pdf_requires_ocr_async(pdf_bytes)
            if requires_ocr and not PADDLEOCR_URL:
            ...
            for batch_start in range(0, job.total_pages, batch_size):
                ...
                # Render only the current batch to avoid storing entire PDF in memory
                batch = await render_pdf_batch_async(pdf_bytes, batch_start, batch_size)
