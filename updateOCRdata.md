...
Use a bounded concurrency worker pool to prevent memory exhaustion and browser crashes. Concurrency should be scaled based on `navigator.hardwareConcurrency` (capped at 4 workers) to prevent memory exhaustion and browser crashes. Terminate workers when a document is complete or cancelled.
Use bounded chunked processing in the browser to prevent massive memory usage (crashing when creating hundreds of high-resolution canvas elements at once). Do not retain large objects (like pdf.js page instances) in memory arrays; explicitly clean them up (e.g., `page.cleanup()`) to allow garbage collection. Process OCR pages in batches equal to `navigator.hardwareConcurrency` (capped at 4). Terminate workers when a document is complete or cancelled.
