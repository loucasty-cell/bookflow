...
Use a bounded concurrency worker pool to prevent memory exhaustion and browser crashes. Concurrency should be scaled based on `navigator.hardwareConcurrency` (capped strictly at 4 workers) to prevent memory exhaustion and browser crashes. Terminate workers when a document is complete or cancelled.
Use bounded chunked processing in the browser to prevent massive memory usage (crashing when creating hundreds of high-resolution canvas elements at once). Only load the required page image at execution time and call `cleanup()` immediately.
