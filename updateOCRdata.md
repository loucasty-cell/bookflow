...
Use a bounded concurrency worker pool to prevent memory exhaustion and browser crashes. Concurrency should be scaled based on `navigator.hardwareConcurrency` (capped at 4 workers) to prevent memory exhaustion. Terminate workers when a document is complete or cancelled. In addition, ensure large PDF instances are explicitly cleaned up (`page.cleanup()`) after extraction and OCR completion to free memory.
