...
export async function parsePdf(file, onProgress) {
  ...
  if (totalPagesForOcr > 0) {
    ocrPageCount = await performLocalOcr(pdf, pagesNeedingOcr, totalPagesForOcr, pdf.numPages, reportProgress);
  } else {
...
async function extractNativeText(pdf, documentTitle, startTime, reportProgress) {
  ...
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
    ...
    // FIX MEMORY LEAK: Do not store the page object in the array. Call cleanup.
    page.cleanup();
...
async function performLocalOcr(pdf, pagesNeedingOcr, totalPagesForOcr, pdfNumPages, reportProgress) {
  reportProgress(
    26,
  ...
  try {
    ...
    const processNext = async () => {
      while (currentIndex < pagesNeedingOcr.length) {
        ...
        // Fetch the page here for OCR to avoid keeping all pages in memory
        const page = await pdf.getPage(pageData.pageNumber);
        const recognized = await recognizePdfPage(ocrScheduler, page);
        page.cleanup(); // Clean up immediately after recognizing
